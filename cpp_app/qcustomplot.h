...
void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    QCPDataContainer<DataType>::iterator itEnd = std::lower_bound(begin(),
                                                                  end(),
...
{
    QCPDataContainer<DataType>::iterator it = std::upper_bound(begin(),
                                                               end(),
                                                               DataType::fromSortKey(sortKey),
                                                               qcpLessThanSortKey<DataType>);
...
void QCPDataContainer<DataType>::remove(double sortKeyFrom, double sortKeyTo)
{
    ...
    QCPDataContainer<DataType>::iterator it = std::lower_bound(begin(),
                                                               end(),
                                                               DataType::fromSortKey(sortKeyFrom),
                                                               qcpLessThanSortKey<DataType>);
    QCPDataContainer<DataType>::iterator itEnd = std::upper_bound(it,
                                                                  end(),
...
{
    QCPDataContainer::iterator it = std::lower_bound(begin(),
                                                     end(),
                                                     DataType::fromSortKey(sortKey),
                                                     qcpLessThanSortKey<DataType>);
    ...
    double sortKey, bool expandedRange) const
{
    ...
    QCPDataContainer<DataType>::const_iterator it = std::lower_bound(constBegin(),
                                                                     constEnd(),
                                                                     DataType::fromSortKey(sortKey),
                                                                     qcpLessThanSortKey<DataType>);
    if (expandedRange
    ...
    double sortKey, bool expandedRange) const
{
    ...
    QCPDataContainer<DataType>::const_iterator it = std::upper_bound(constBegin(),
                                                                     constEnd(),
                                                                     DataType::fromSortKey(sortKey),
                                                                     qcpLessThanSortKey<DataType>);
...
public:
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details = 0) const
        Q_DECL_OVERRIDE = 0; // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for plottables
...
{
    if (index >= 0 && index < mDataContainer->size()) {
        const typename QCPDataContainer<DataType>::const_iterator it = mDataContainer->constBegin()
                                                                       + index;
                                                                  ...
                                                                  bool onlySelectable) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        {
            result.addDataRange(QCPDataRange(currentSegmentBegin, it - mDataContainer->constBegin()),
                                false);
                                                    ...
                                                    QVariant *details) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        {
            const double currentDistSqr = QCPVector2D(coordsToPixels(mainKey, mainValue) - pos)
                                              .lengthSquared();
